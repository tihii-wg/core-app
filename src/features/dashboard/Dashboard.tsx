    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <DashboardCard
          ...
          value={lowStockItems}
          icon={Package}
          variant={lowStockItems > 0 ? 'danger' : 'default'}
        />
      </div>
      ...
      <div className="bg-white rounded-md border border-[#eeeeef] p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            onClick={() => setCurrentModule('clients')}
            variant="outline"
            size="sm"
            className="border-[#c9cbcc]"
          >
            <Plus className="h-4 w-4 mr-1" />
            Add Client
          </Button>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => setCurrentModule('orders')}
              className="text-sm text-[#1973e1] hover:underline flex items-center gap-1"
            >
              View all
            ...
            <div className="mt-4 pt-4 border-t border-[#eeeeef]">
              <div className="flex items-end justify-between gap-1 h-16">
                {[40, 65, 45, 80, 55, 90, 70].map((height, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-[#edf4fd] hover:bg-[#1973e1] transition-colors rounded-sm cursor-pointer"
                    style={{ height: `${height}%` }}
                  />
                ))}
              </div>
