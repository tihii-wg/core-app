...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      {description && (
        <p className="text-sm text-[#939699] mt-1 max-w-sm">{description}</p>
      )}
      {action && (
        <Button
          onClick={action.onClick}
          className="mt-4 bg-[#1973e1] hover:bg-[#1565c0] text-white"
        >
          {action.label}
        </Button>
      )}
...
export function NoOrders({ onCreateOrder }: { onCreateOrder: () => void }) {
  return (
    <EmptyState
      ...
      action={{ label: 'Create Order', onClick: onCreateOrder }}
    />
  );
}
